...
export default function DispatchCenterPage() {
  ...
  // Persistent cross-page participant-to-userId mapping
  const allParticipantUserMap = useRef(new Map<string, number>());

  // Clear stale email preview when options change
  useEffect(() => {
    setEmailPreview(null);
  }, [includeLink, includeMessage, messageText]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          setViewData(res);
          // Accumulate participant-to-userId mapping across pages
          for (const p of res.participants?.items ?? []) {
            if (p.participant_id && p.user_id != null) {
              allParticipantUserMap.current.set(p.participant_id, p.user_id);
            }
          }
        }
      } catch (e: any) {
  ...
  async function handleConfirmDispatch() {
    ...
    if (isNaN(postingId)) {
      setDispatchError('Invalid study ID');
    ...
    try {
      const user_ids = Array.from(selectedParticipantIds)
        .map((pid) => allParticipantUserMap.current.get(pid))
        .filter((uid): uid is number => uid != null);

      const unmapped = selectedParticipantIds.size - user_ids.length;
      if (unmapped > 0) {
        setDispatchError(`Warning: ${unmapped} participant(s) could not be included because they are not linked to user accounts.`);
  ...
  return (
    <SafeAreaView style={dStyles.root}>
      <ScrollView contentContainerStyle={dStyles.container} keyboardShouldPersistTaps="handled">
        {/* ── Header ── */}
        <View style={dStyles.pageHeader}>
          <View style={dStyles.headerTitleWrap}>
          ...
          <View style={[dStyles.headerActions, isNarrow && { width: "100%" }]}>
            ...
            >
              <Text style={dStyles.btnSecondaryText}>Message History</Text>
            </TouchableOpacity>
            <TouchableOpacity
        ...
        {(surveysLoading || viewLoading) && (
          <ActivityIndicator
            ...
            style={dStyles.loadingIndicator}
          />
        )}
            ...
            <View style={dStyles.resultGrid}>
              {(
                [
                  ...
                  ...(dispatchResult.results.message_event_id != null
                    ? [["Msg Event ID", dispatchResult.results.message_event_id] as [string, number]]
                    : []),
                  ...(dispatchResult.results.message_recipients_created != null
                    ? [["Msg Recipients", dispatchResult.results.message_recipients_created] as [string, number]]
                    : []),
                ] as [string, number][]
              ).map(([label, val]) => (
        ...
        {dispatchError && (
          <View style={dStyles.errorBanner}>
            ...
            <TouchableOpacity onPress={() => setDispatchError(null)}>
              <Text style={dStyles.retryText}>Dismiss</Text>
            </TouchableOpacity>
          </View>
        )}
                ...
                surveys.map((s) => (
                  ...
                  >
                    ...
                    <View style={dStyles.checkRowContent}>
                      ...
                      <View style={dStyles.checkRowBadgeRow}>
                        <View
                          style={[
                            dStyles.miniBadge,
                            s.is_active
              ...
              <View style={dStyles.panelHeader}>
                ...
                <View style={dStyles.panelControls}>
                  <TouchableOpacity onPress={selectAllParticipantsOnPage}>
                    <Text style={dStyles.controlLink}>Select Page</Text>
                  </TouchableOpacity>
                  <Text style={dStyles.controlSep}>·</Text>
        ...
        {/* ── Email Options ── */}
        {dataLoaded && (
          <View style={dStyles.emailOptionsSection}>
            <Text style={dStyles.emailOptionsSectionTitle}>Email Options</Text>

            {/* Include Link Toggle */}
            <TouchableOpacity
              style={dStyles.toggleRow}
            ...
            >
              <View
                style={[
                  dStyles.checkBox,
                  includeLink && dStyles.checkBoxChecked,
                ]}
              >
                {includeLink && (
                  <Text style={dStyles.checkMark}>✓</Text>
              ...
              <View style={dStyles.toggleRowContent}>
                <Text style={dStyles.toggleRowLabel}>Include Survey Link</Text>
                <Text style={dStyles.toggleRowSubtext}>
                  Attach a personalized survey link in the email
                </Text>
              </View>
            </TouchableOpacity>
            ...
            >
              <View
                style={[
                  dStyles.checkBox,
                  includeMessage && dStyles.checkBoxChecked,
                ]}
              >
                {includeMessage && (
                  <Text style={dStyles.checkMark}>✓</Text>
              ...
              <View style={dStyles.toggleRowContent}>
                <Text style={dStyles.toggleRowLabel}>Include Custom Message</Text>
                <Text style={dStyles.toggleRowSubtext}>
                  Add a custom message body to the email
                </Text>
              </View>
            </TouchableOpacity>
            ...
            {includeMessage && (
              <TextInput
                style={dStyles.messageTextInput}
                placeholder="Enter custom message text…"
                placeholderTextColor={palette.light.text.muted}
                value={messageText}
            ...
            {/* Preview Button */}
            <TouchableOpacity
              style={[dStyles.btnPreview, previewLoading && dStyles.btnDisabled]}
              onPress={handlePreviewEmail}
              disabled={previewLoading}
            >
              {previewLoading ? (
                <ActivityIndicator color={palette.light.primary} size="small" />
              ) : (
                <Text style={dStyles.btnPreviewText}>Preview Email</Text>
              )}
            </TouchableOpacity>

            {/* Preview Error */}
            {previewError && (
              <View style={dStyles.previewErrorBox}>
                <Text style={dStyles.previewErrorText}>{previewError}</Text>
              </View>
            )}
        ...
        <View style={dStyles.modalOverlay}>
          <View style={dStyles.modalCard}>
            ...
            {[
              ...
              ["Include Link", includeLink ? "Yes" : "No"],
              ["Include Message", includeMessage ? "Yes" : "No"],
            ].map(([label, val]) => (
...
const dStyles = StyleSheet.create({
  ...
  checkRowBadgeRow: { flexDirection: "row", gap: 8, alignItems: "center" },
