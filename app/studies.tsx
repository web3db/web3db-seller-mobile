    ...
    useCallback(() => {
      const loadStudies = async () => {
        ...
        try {
          ...
          setError(
            `Failed to load studies: ${e.message || "Unknown network error"}`
          );
          setStudies([]);
        } finally {
          setIsLoading(false);
      ...
      <ScrollView contentContainerStyle={styles.homeRoot}>
        <View style={styles.header}>
          <View style={styles.leftColumn}>
            ...
            <Text style={styles.subtitle}>
              Below are the studies your organization currently manages. Click a
              study to view details or manage recruitment.
            </Text>
          </View>
          <TouchableOpacity
        ...
        <View style={styles.stats}>
          <StatBox
            ...
            value={isLoading ? "..." : totalActiveStudies}
          />
        </View>
        ...
        <View style={styles.listContainer}>
          {isLoading && (
            <Text style={styles.message}>Loading studies via fetch...</Text>
          )}
          {error && (
            <Text style={[styles.message, styles.errorText]}>⚠️ {error}</Text>
          )}

          {!isLoading && studies.length === 0 && !error && (
            <Text style={styles.message}>
              No studies found. Start by adding a new one!
            </Text>
          )}
          ...
          {studies.map((study) => (
            <StudyCard
              ...
              study={study}
              statusLabel={
                statusLabelById.get(study.statusId) ??
                `Status ${study.statusId}`
              }
